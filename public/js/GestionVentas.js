      ...
      $.ajax({
        ...
        success: function (data)   // Una función a ser llamada si la solicitud tiene éxito
        {
          //console.log(data);
            ...
            if (data.items == 0) {
              $(".frmVentas_modal").modal('hide');
            }else{
              console.log("dddddd",data);
              var total=Number(`${data.items.total}`).toFixed(2);
              var fila = `
      <div class="col bg-warning"><strong>Fecha de venta:</strong></div>           <div class="col">${data.items.fechaOrden}</div>
      <div class="col  bg-warning"><strong>Total:</strong></div>           <div class="col">$ ${total}</div>
        <div class="w-100"></div>
      <div class="col bg-warning"><strong>Nombre del cliente :</strong></div>           <div class="col">${data.items.cliente.name}</div>
      <div class="col bg-warning"><strong>Transportista :</strong></div>           <div class="col">${data.items.courier.name}</div>
